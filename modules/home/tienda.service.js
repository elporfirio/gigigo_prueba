...
(function () {
    ...
    function TiendaService($http, postRequestFix, putRequestFix, $window, Json2FormService, $httpParamSerializerJQLike, env) {
        ...
        function saveProducto(producto){
            if(producto.id === null){
                var form = Json2FormService.convertJson2From(producto);
                return $http.post(env.api + 'product?token='+ $window.localStorage.token, form, postRequestFix)
                    .then(saveProductoSuccess)
                    .catch(saveProductoFailed);
            } else {
                return $http.put(env.api + 'product/' + producto.id +'?token='+ $window.localStorage.token,
        ...
        function deleteProducto(producto){
            return $http.delete(env.api + 'product/' + producto.id +'?token='+ $window.localStorage.token, postRequestFix)
        ...
        function detailProducto(producto) {
            return $http.get(env.api + 'product/'+ producto.id +'?token=' + $window.localStorage.token )
                .then(getProductosSuccess)
                .catch(getProductosFailed);
